...
**Design note (hybrid, not relay-only):** an earlier draft of this plan said "the relay is the only publisher." That was wrong — it would make every alert wait for the next relay tick, regressing the headline latency. The shipped design publishes inline and uses the relay only as a backstop. (Credit: caught in a second-opinion review.) Guarantee is **at-least-once**: if a publish succeeds but the row isn't marked published — a crash, *or* the 2s relay sweep landing in the small gap between `Publish` and the mark — the relay re-sends, so at most one duplicate. The alert path and the 48h freshness gate tolerate that. A `created_at` grace window on the relay query (`AND created_at < now() - interval '10s'`) would shrink the duplicate source to crash-only; deliberately not added — the duplicate is rare and harmless at single-user volume, and the grace mechanism would be the kind of gold-plating DESIGN §1 warns against. Revisit if duplicate alerts ever become noticeable.
