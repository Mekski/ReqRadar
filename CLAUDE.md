...
1a. **"Expected open" accuracy — KEY OPEN DECISION (Mark, 2026-06-15).** Cards/detail now show an "expected open" month = peak month-of-year of **summer + SWE** posting_opened (the `is_summer` + category filters, added 2026-06-15). The summer filter fixed high-volume companies (Microsoft→Oct, Riot→Oct, Meta→Sep — correct). BUT it's **noisy for low-volume companies** (Roblox→Mar but truth is Jul/Aug; Tesla→Jun) because 1–2 postings can't form a pattern and aggregator posted-dates mislead. **Recommended next step:** confidence-gate it — only trust the data-derived month when a company has ≥~5 summer-SWE postings; otherwise show "—" OR a clearly-labeled LLM estimate (we have the Claude API — one cached call: "when does {company} open summer SWE intern apps?"). Do NOT hardcode Google answers (doesn't scale, weak story). Graph stays the honest historical-actual. Mark leaned toward filters-vs-Google; the evidence says hybrid (data when dense, LLM-est. labeled when sparse).
