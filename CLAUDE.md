...
1a. **"Expected open" — DECISION MADE (2026-06-16).** Cards/detail show expected-open = peak month of **summer + SWE** posting_opened (`is_summer` + category filters).
- **Confidence gate IS IMPLEMENTED** (`attachExpected` in `internal/store/api.go`, `minSamples=5`): a data-derived month shows only when a company has ≥5 summer-SWE postings. Currently 9/20 qualify (Amazon→Dec, Apple→May, EA→Oct, Epic→Oct, Meta→Sep, Microsoft→Oct, NVIDIA→Sep, Tesla→Jun, TikTok→Aug); the other 11 (Anthropic, Databricks, GitHub, Google, LinkedIn, Niantic, Notion, OpenAI, Riot, Roblox, xAI) are blank → need the fallback.
- **Fallback = CURATED SEED estimates, NOT a runtime LLM** (Mark's call, confirmed). Expected-open is *static reference data*, so: research each sparse company's summer-SWE open window once (thorough + cited) and seed it as `expected_estimate` in `seed/watchlist.yaml`. UI shows data-month → else estimate labeled **"≈ est."** → else "—". Rejected a runtime LLM here as over-engineering for static data (grounded research is more reliable, free, reviewable, versioned). Do NOT build an LLM call for this.
- **NEXT SESSION (the active task):** (1) research the 11 sparse companies' summer SWE-intern open windows; (2) add `expected_estimate` to the seed YAML + `cmd/seed` loader (store in `entities.metadata`) + an `expected_estimate` field on `/api/companies` + the "≈ est." label in `CompanyCard.tsx`.
- **Future genuine LLM (sentiment summaries / fuzzy entity disambiguation, DESIGN §3.4): use a FREE tier** — Gemini Flash (1,500 req/day, no card), Groq, Cerebras (1M tok/day), Mistral (1B tok/mo). Mark won't pay; **no Anthropic paid API** (`ANTHROPIC_API_KEY` is intentionally unset). See `prefers-free-tooling` memory.
